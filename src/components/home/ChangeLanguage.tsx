...
import { localeList } from "~/server/translation";
import { useClickOutside } from "~/utils/useClickOutside";

export const ChangeLanguage = () => {
  const [showLangueageMenu, setShowLanguageMenu] = useState(false);
    ...
    <div className="relative" ref={dropDownRef}>
      ...
      {showLangueageMenu && (
        <ul className="absolute right-0 shadow">
          {Object.entries(localeList).map(([key, name]) => {
            return (
              <Link
                key={key}
                onClick={() => {
                ...
                locale={key}
              >
                <li className="cursor-pointer whitespace-nowrap px-5 py-3 hover:bg-blue-300">
                  {name}
                </li>
              </Link>
            );
