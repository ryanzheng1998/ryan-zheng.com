...
const Clipper2ZUtilsFactory =
  _Clipper2ZUtilsFactory as unknown as Clipper2ZFactoryFunction

export default function Page() {
  const [clipper, setClipper] = useState<MainModule | null>(null)
  const [clipperUtils, setClipperUtils] = useState<MainModule | null>(null)
  ...
  useEffect(() => {
    const main = async () => {
      ...
      const utilsMod = await Clipper2ZUtilsFactory({
        locateFile: () => '/clipper2z-utils.wasm',
      })
      setClipperUtils(utilsMod)
      setClipper(mod)
    }
